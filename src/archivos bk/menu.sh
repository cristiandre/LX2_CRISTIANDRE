...
gnuplot << EOF 2> /home/sysadmin/lx2_cristiandre/logs/error_graf2.log
set terminal png
set output "$C"
set grid
set xtics 1,1,5 #inicio, incremento, final
set xrange [0:5] #inicio, fin
set xtics 1
...
t2="Indice de radiacion KW/m2 Min"

gnuplot << EOF 2> /home/sysadmin/lx2_cristiandre/logs/error_graf3.log
xdata time
timefmt " %Y %m %d %H %M"
xrange ["$FMT_BEGIN" : "$FMT_END"]
set terminal png
set output "$C"
plot "$F" using 1 with lines title "$t1",\
...
menu_principal() {
	...
	echo "1. Problema2 Gastos"

	echo "2. Problema3 Max - Min"
